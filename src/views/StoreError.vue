<template>
  <div class="page">
    <div class="heading"><div><h2>错误回店</h2><p>仅管理员可登记和处理。本店异常订单会冻结；错店订单不能在本店签收。</p></div><el-button @click="load">刷新</el-button></div>
    <el-form :model="form" label-width="90px" class="form" @submit.prevent>
      <el-form-item label="订单号"><el-input v-model.trim="form.orderNo" placeholder="扫描或输入订单号" /></el-form-item>
...
async function create(){await ElMessageBox.confirm('确认登记异常？本店该订单所有待签收衣物将被冻结。','登记错误回店',{type:'warning'});await storeOperationsApi.createError({...form});form.orderNo='';form.description='';ElMessage.success('异常已登记');await load()}
